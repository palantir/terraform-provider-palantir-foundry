...
var v0RootTfType = tftypes.Object{
	AttributeTypes: map[string]tftypes.Type{
		"organization_rid":              tftypes.String,
		"organization_role_assignments": tftypes.Set{ElementType: v0EntryTfType},
	},
}
...
var v1RootTfType = tftypes.Object{
	AttributeTypes: map[string]tftypes.Type{
		"organization_rid": tftypes.String,
		"organization_role_assignments": tftypes.Map{
...
func testV1Schema() schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"organization_role_assignments": helper.RoleAssignmentMapSchema("Map of Role ID to set of Principal IDs."),
		},
	}
}
...
func buildV0State(rid string, assignments []v0Assignment) tfsdk.State {
	...
	raw := tftypes.NewValue(v0RootTfType, map[string]tftypes.Value{
		"organization_rid":              tftypes.NewValue(tftypes.String, rid),
		"organization_role_assignments": tftypes.NewValue(tftypes.Set{ElementType: v0EntryTfType}, entries),
	})
...
func buildEmptyV1State() tfsdk.State {
	raw := tftypes.NewValue(v1RootTfType, map[string]tftypes.Value{
		"organization_rid": tftypes.NewValue(tftypes.String, ""),
		"organization_role_assignments": tftypes.NewValue(
...
func TestUpgradeStateV0ToV1(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			organizationRID: "ri.organization..abc123",
			v0Assignments: []v0Assignment{
				{roleID: "organization:administrator", principalID: "user-1"},
			},
			expectedRoles: map[string][]string{
				"organization:administrator": {"user-1"},
			},
		},
		{
			...
			v0Assignments: []v0Assignment{
				{roleID: "organization:administrator", principalID: "user-1"},
				{roleID: "organization:administrator", principalID: "user-2"},
				{roleID: "organization:administrator", principalID: "user-3"},
			},
			expectedRoles: map[string][]string{
				"organization:administrator": {"user-1", "user-2", "user-3"},
			},
		},
		{
			...
			v0Assignments: []v0Assignment{
				{roleID: "organization:administrator", principalID: "user-1"},
				{roleID: "organization:viewer", principalID: "user-2"},
				{roleID: "organization:administrator", principalID: "user-3"},
				{roleID: "organization:viewer", principalID: "user-4"},
				{roleID: "organization:editor", principalID: "user-5"},
			},
			expectedRoles: map[string][]string{
				"organization:administrator": {"user-1", "user-3"},
				"organization:viewer":        {"user-2", "user-4"},
				"organization:editor":        {"user-5"},
			},
		},
		{
		...
		{
			name:            "organization rid is preserved",
			organizationRID: "ri.organization..some-specific-rid-value",
			v0Assignments: []v0Assignment{
				{roleID: "organization:administrator", principalID: "user-1"},
			},
			expectedRoles: map[string][]string{
	...
	r := &organizationRoleAssignmentsResource{}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			reqState := buildV0State(tt.organizationRID, tt.v0Assignments)
			...
			// Verify organization_rid is preserved
			var gotRID string
			diags := resp.State.GetAttribute(ctx, path.Root("organization_rid"), &gotRID)
			if diags.HasError() {
				t.Fatalf("failed to read organization_rid from upgraded state: %v", diags.Errors())
			}
			if gotRID != tt.organizationRID {
				t.Errorf("organization_rid: got %q, want %q", gotRID, tt.organizationRID)
			}

			// Verify role assignments map
			var gotMap types.Map
			diags = resp.State.GetAttribute(ctx, path.Root("organization_role_assignments"), &gotMap)
			if diags.HasError() {
				t.Fatalf("failed to read organization_role_assignments from upgraded state: %v", diags.Errors())
			}
